...
void AP_MotorsCoax::output_to_motors()
{
    switch (_spool_mode) {
        case SHUT_DOWN:
            ...
            hal.rcout->cork();
            rc_write(AP_MOTORS_MOT_1, calc_pwm_output_1to1(+ _pitch_radio_passthrough + _yaw_radio_passthrough , _servo1));
...
// Main rotor servos
actuator[0] = rp_scale*pitch_thrust + yaw_thrust + lateral_thrust; // Right
actuator[1] = - rp_scale*pitch_thrust + yaw_thrust + lateral_thrust; // Left
// Flap servos
actuator[2] = -(pitch_thrust + roll_thrust)/2; //Right
actuator[3] = (pitch_thrust - roll_thrust)/2; //Left
